...
public class SpawnerData {
    ...
    /**
     * True when the hologram is intentionally absent due to the distance gate in
     * HologramVisibilityTask. Prevents refresh/reload paths from resurrecting it.
     */
    private volatile boolean hologramSuppressed = false;
    ...
    public void reloadHologramData() {
        ...
        if (!hologramSuppressed
                && plugin.getConfig().getBoolean("hologram.enabled", false)) {
            createHologram();
        }
    }
    ...
    public void refreshHologram() {
        if (plugin.getConfig().getBoolean("hologram.enabled", false)) {
            if (hologram == null && !hologramSuppressed) {
                createHologram();
            }
    ...
    /** @return true when the distance gate has intentionally removed this spawner's hologram. */
    public boolean isHologramSuppressed() {
        return hologramSuppressed;
    }

    /**
     * Called by {@code HologramVisibilityTask} when no player is within
     * {@code hologram.show_radius}. Removes the live TextDisplay immediately; the flag prevents
     * refresh/reload paths from resurrecting it until {@link #unsuppressHologram()}.
     */
    public void suppressHologram() {
        hologramSuppressed = true;
        removeHologram();
    }

    /** Called when a player re-enters range, or on plugin reload. Re-creates the hologram if enabled. */
    public void unsuppressHologram() {
        hologramSuppressed = false;
        if (plugin.getConfig().getBoolean("hologram.enabled", false)) {
            refreshHologram();
        }
    }
