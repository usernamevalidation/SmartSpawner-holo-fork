...
public class SpawnerSellManager {
    ...
    public void sellAllItems(Player player, SpawnerData spawner) {
        sellAllItems(player, spawner, null, 0, 0, "GUI");
    }
    ...
    public void sellAllItems(Player player, SpawnerData spawner, Runnable onComplete) {
        sellAllItems(player, spawner, onComplete, 0, 0, "GUI");
    }

    /**
     * Sells all items from the spawner's virtual inventory.
     * Convenience overload with a caller-supplied source tag.
     */
    public void sellAllItems(Player player, SpawnerData spawner, String source) {
        sellAllItems(player, spawner, null, 0, 0, source);
    }

    /**
     * Sells all items from the spawner's virtual inventory.
     * Backwards-compatible overload without a source tag; defaults to {@code "GUI"}.
     */
    public void sellAllItems(Player player, SpawnerData spawner, Runnable onComplete, long expCollected, long expMending) {
        sellAllItems(player, spawner, onComplete, expCollected, expMending, "GUI");
    }
     ...
     * @param source       free-form source tag forwarded to {@link SpawnerSellEvent#setSource(String)}.
     */
    public void sellAllItems(Player player, SpawnerData spawner, Runnable onComplete, long expCollected, long expMending, String source) {
        ...
        Scheduler.runLocationTask(spawnerLocation, () -> {
            try {
                applySellResult(player, spawner, result, expCollected, expMending, source);
            } finally {
     ...
     * @param source       free-form source tag passed to the fired {@link SpawnerSellEvent}
     */
    private void applySellResult(Player player, SpawnerData spawner, SellResult sellResult, long expCollected, long expMending, String source) {
        ...
        if (debug) {
            plugin.getLogger().info("[DEBUG] Sell: player=" + player.getName()
                    + " loc=" + formatLoc(spawner.getSpawnerLocation())
                    + " source=" + source
                    + " base=" + amount);
        }
        ...
        if (SpawnerSellEvent.getHandlerList().getRegisteredListeners().length != 0) {
            ...
            if (event.isCancelled()) {
                if (debug) {
                    plugin.getLogger().info("[DEBUG] Sell: cancelled by listener, aborting. source=" + source);
                }
                return;
            }
            if (event.getMoneyAmount() >= 0) {
                if (debug && event.getMoneyAmount() != amount) {
                    plugin.getLogger().info("[DEBUG] Sell: amount overridden by listener "
                            + amount + " -> " + event.getMoneyAmount() + " source=" + source);
                }
        ...
        if (!depositSuccess) {
            if (debug) {
                plugin.getLogger().info("[DEBUG] Sell: deposit failed for " + player.getName() + " amount=" + amount);
            }
            messageService.sendMessage(player, "action_failed");
            return;
        }
        ...
        if (debug) {
            plugin.getLogger().info("[DEBUG] Sell: completed player=" + player.getName()
                    + " source=" + source
                    + " paid=" + amount
                    + " items_sold=" + sellResult.getItemsSold());
        }
