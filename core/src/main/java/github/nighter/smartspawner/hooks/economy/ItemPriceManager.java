...
public class ItemPriceManager {
    ...
    /**
     * Gets the price a specific player would receive for the given material.
     * <p>
     * When the active shop integration supports player-aware pricing
     * (e.g. EconomyShopGUI Premium rank or permission multipliers), that value
     * is used. Otherwise this falls back to {@link #getPrice(Material)}.
     * <p>
     * Custom-price modes are intentionally NOT player-aware — a custom price is a
     * fixed override and rank multipliers should not apply on top of it.
     *
     * @param material the material to price
     * @param player the player the price is for
     * @return the player-aware unit price, or {@code 0.0} if unavailable
     */
    public double getPrice(Material material, Player player) {
        if (material == null || !economyEnabled) return 0.0;
        if (player == null) return getPrice(material);

        switch (priceSourceMode) {
            case CUSTOM_ONLY:
                return getCustomPrice(material);
            case SHOP_ONLY:
                return getPlayerShopPrice(material, player);
            case CUSTOM_PRIORITY: {
                double customPrice = getCustomPrice(material);
                return customPrice > 0 ? customPrice : getPlayerShopPrice(material, player);
            }
            case SHOP_PRIORITY: {
                double shopPrice = getPlayerShopPrice(material, player);
                return shopPrice > 0 ? shopPrice : getCustomPrice(material);
            }
            default:
                return defaultPrice;
    ...
    private double getPlayerShopPrice(Material material, Player player) {
        if (!economyEnabled || !shopIntegrationEnabled || shopIntegrationManager == null) return 0.0;
