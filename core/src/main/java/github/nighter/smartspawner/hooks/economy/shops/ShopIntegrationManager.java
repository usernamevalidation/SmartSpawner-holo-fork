...
public class ShopIntegrationManager {
     ...
     * <p>
     * Delegates to the active {@link ShopProvider}'s player-aware method if it
     * supports one. If the provider returns {@code 0.0} for the player-aware
     * path (for example because it has no rank/permission multiplier configured),
     * this falls back to the non-player value so behavior is unchanged for
     * providers that don't care about the player.
     *
    ...
    public double getPrice(Material material, Player player) {
        ...
        try {
            double playerPrice = activeProvider.getSellPrice(material, player);
            if (playerPrice > 0.0) {
                return playerPrice;
            }
            return getPrice(material);
